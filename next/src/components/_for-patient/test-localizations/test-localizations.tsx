import styles from './test-localizations.module.scss';
import { Props } from './test-localizations.constants';
...
export default function TestLocalizations({ title, text, cards }: Props) {
  return (
    <section id='gdzie-wykonac-badania' className={`${styles.wrapper} container`}>
      <div>
        <Markdown.h2
          children={title}
      ...
      <div className={styles.grid}>
        {cards.map((el, i) => (
          <div
            key={el.title + i}
            className={`${styles.card} ${el.isClosed ? styles.closing : ''}`}
          >
            <Markdown.h3
              children={el.title}
              className={`${styles.cardTitle} h4`}
            />
